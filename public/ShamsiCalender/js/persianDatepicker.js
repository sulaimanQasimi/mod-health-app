...
(function ($) {
  ...
  var persianDatepicker = (function () {
      function persianDatepicker(element, userOptions) {
          ...
          var _fontSize = options.fontSize;
          var _cw = parseInt(options.cellWidth);
          var _ch = parseInt(options.cellHeight);
          ...
          if (self.options.selectedDate == undefined && !self.options.showGregorianDate) {
              var patt1 = new RegExp('^([1-9][0-9]{3})/([0]?[1-9]|1[0-2])/([0]?[1-9]|[12][0-9]|3[01])$');
              if (el.is('input')) {
          ...
          if (options.selectableYears && options.selectableYears.indexOf(self._persianDate.year) === -1)
              self._persianDate.year = options.selectableYears[0];
          if (options.selectableMonths.indexOf(self._persianDate.month) === -1)
          ...
          el.addClass('pdp-el')
              .on('click', function (e) { self.show(e); })
              .on('focus', function (e) { self.show(e); });
          ...
          function getFormattedDate(dateObj) {
              return dateObj.year + '/' + padZero(dateObj.month) + '/' + padZero(dateObj.date);
          }

          // --- selectedBefore handling ---
          if (options.selectedBefore) {
              var dateToShow = self.options.selectedDate ? self.persianDate.parse(self.options.selectedDate) : self.now();
              self.showDate(el, getFormattedDate(dateToShow) + " " + self.jDateFunctions.getWeekday(dateToShow), self.now().gDate, options.showGregorianDate);
          }

          if (options.isRTL) el.addClass('rtl');
          if (self.calendar.length && !options.alwaysShow) self.calendar.hide();

          $(document).bind('mouseup', function (e) {
              var target = e.target;
              var calendar = self.calendar;
              if (!el.is(target) && !calendar.is(target) && calendar.has(target).length === 0 && calendar.is(':visible')) self.hide();
    ...
    persianDate.prototype = {
        ...
        toString: function (formatDate) {
            const replacements = {
                ...
                "MM": pad(this.month, 2),
                "DD": pad(this.date, 2),
                "hh": this.gDate.getHours(),
                "mm": this.gDate.getMinutes(),
                "ss": this.gDate.getSeconds(),
                "0h": pad(this.gDate.getHours(), 2),
                "0m": pad(this.gDate.getMinutes(), 2),
                "0s": pad(this.gDate.getSeconds(), 2),
                "tm": (this.gDate.getHours() >= 12 && this.gDate.getMinutes() > 0) ? "ب.ظ" : "ق.ظ",
                "ms": this.gDate.getMilliseconds(),
                "NM": this.months[this.month - 1],
                "DW": this.day,
                "ND": this.dowTitle[this.day]
            };

            if (!formatDate) {
                return `${replacements.YYYY}/${replacements.MM}/${replacements.DD}`;
            }

            return formatDate.replace(/\b(YYYY|MM|DD|hh|mm|ss|0h|0m|0s|tm|ms|NM|DW|ND)\b/g, key => replacements[key]);
        }
    };
