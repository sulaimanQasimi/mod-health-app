...
(function () {
  ...
  var persianDatepicker = (function () {
    function persianDatepicker(element, userOptions) {
      ...
      self.selectUlStyle =
        ...
        self.options.cellHeight +
        "px;height:" +
        (self.options.cellHeight * 7 + 20) +
        "px; font-size:" +
        (_fontSize - 2) +
        "px;' ";
      self.selectMonthLiStyle =
        "style='height:" +
        (self.options.cellHeight * 7 + 7) / 4 +
        "px;line-height:" +
        (self.options.cellHeight * 7 + 7) / 4 +
        "px; width:" +
        (7 * self.options.cellWidth + 1) / 3 +
        "px;width:" +
        (7 * self.options.cellWidth) / 3 +
        "px\\9;' ";
      self.selectYearLiStyle =
        "style='height:" +
        (self.options.cellHeight * 7 + 10) / 6 +
        "px;line-height:" +
        (self.options.cellHeight * 7 + 10) / 6 +
        "px; width:" +
        (7 * self.options.cellWidth - 14) / 3 +
        "px;width:" +
        (7 * self.options.cellWidth - 15) / 3 +
        "px\\9;' ";
      self.footerStyle =
        "style='height:" +
        self.options.cellHeight +
      ...
      if (
        self.options.selectableYears != undefined &&
        self.options.selectableYears._indexOf(_persianDate.year) == -1
      )
      ...
      self.calendar = $(
        '<div id="' + self._id + '" class="pdp-' + self.options.theme + '" />'
      );
        ...
        .bind("focus", function (e) {
          ...
          self.calendar.css({
            top:
              elPos.top +
              el.outerHeight() +
              self.options.calendarPosition.y +
              "px",
      ...
      if (self.options.isRTL) el.addClass("rtl");
      if (self.calendar.length && !options.alwaysShow) {
        self.calendar.hide();
      }
      // el.bind('change focus keyup', function (e) {
      //     var md=new persianDate;
      //      md=md.parse(md.parse(self.el.val()).toString("DD-MM-YYYY"));
      //      md.gDate= self.jDateFunctions.getGDate(md);
      //     self.persianDate = md;
      //     self._persianDate = md;
      //     self.render();

      //    //console.log (self.persianDate);
      // });
    ...
    persianDatepicker.prototype = {
      ...
      header: function () {
        ...
        var getSelectableYears = function (f, l) {
          ...
          var arr = [];
          b = 1290;
          for (i = b; i < a && a > 0; i++) arr.push(parseInt(i));
        ...
        $.each(getAllMonths(), function (i, v) {
          var o =
            self.options.selectableMonths._indexOf(i + 1) == -1
              ? $(
                  '<li class="desableMonth" ' + self.selectMonthLiStyle + " />"
                ).html(v)
              : $("<li " + self.selectMonthLiStyle + " />").html(v);
        ...
        _prev = $('<div class="prevArrow" />')
          ...
          .bind("click", function () {
            ...
            for (
              ;
              self.options.selectableMonths._indexOf(prevMonth) == -1 &&
              prevMonth > 0;
              prevMonth--
            );
      ...
      content: function () {
        ...
        jd = self.persianDate;
        jd.date = 1;
        _start = self.jDateFunctions.getWeekday(
          self.jDateFunctions.getJulianDayFromPersian(jd)
        );
        ...
        for (var row = 0, cellIndex = 0; row < 5 + 1; row++) {
          ...
          for (var col = 0; col < 7; col++, cellIndex++) {
            if (cellIndex < _start || cellIndex - _start + 1 > _end) {
              ...
              if (cellIndex - _start + 1 == self.now().date) _selday = "selday";
              _fri = col == 6 ? "friday" : "";
              _cell = $(
                '<div class="day cell ' +
                  _fri +
              ...
              _cell.data("date", {
                // jDate: _dt.toString(" DW-DD-MM-YYYY"),
                jDate: _dt.toString("YYYY-MM-DD-DW"),
                gDate: self.jDateFunctions
                  .getGDate(_dt)
                  ._toString(self.options.formatDate),
              });
              _cell
                .html(
                  self.options.persianNumbers
      ...
      footer: function () {
        ...
        if (
          ...
          _goToday.data("date", {
            jDate: self.now().toString("YYYY-MM-DD-DW"),
            gDate: self.jDateFunctions
              .getGDate(self.now())
              ._toString(self.options.formatDate),
          });
          _goToday
            ...
            .bind("click", function () {
              ...
              if (self.options.showGregorianDate)
                self.showDate(
                  self.el,
                  $(this).data("date").gDate._toString(self.options.formatDate)
                );
              else
                self.showDate(
                  self.el,
      ...
      getDate: function (jd, d) {
        jd.date = d;
        jd.day = this.jDateFunctions.getWeekday(
          this.jDateFunctions.getJulianDayFromPersian(jd)
        );
        return jd;
      },
      now: function () {
        //return this.jDateFunctions.getPCalendarDate(this.persianDate);
        if (!this.el.val()) {
          ...
          var md = new persianDate();
          md = md.parse(md.parse(this.el.val()).toString("DD-MM-YYYY"));
          md.gDate = this.jDateFunctions.getGDate(md);
          //console.log(md);
          //return this.jDateFunctions.getPCalendarDate(this.jDateFunctions.getJulianDay(new Date()));
          return this.jDateFunctions.getPCalendarDate(
            this.jDateFunctions.getJulianDayFromPersian(md)
          );
        }
      },
    };
    ...
    persianDate.prototype = {
      ...
      toString: function (formatDate) {
        ...
        return formatDate
          ...
          .replace("MM", pad(this.month, 2))
          .replace("DD", pad(this.date, 2))
    ...
    jDateFunctions.prototype = {
      ...
      getGDate: function (pd) {
        return this.getGCalendarDate(
          this.getJulianDayFromPersian(pd),
          "gmonth"
        );
      },
      getPCalendarDate: function (jd) {
        ...
        if (jd > 0.0) {
          ...
          if (cyear == 1029982) {
            ...
            var a2 = cyear % 366;
            ycycle =
              Math.floor((2134 * a1 + 2816 * a2 + 2815) / 1028522) + a1 + 1;
          }
      ...
      getJulianDay: function (d, jgGOrJ) {
        ...
        if (
          ...
          ((m0 == 1 ||
            ...
            m0 == 12) &&
            d0 > 31) ||
          ((m0 == 4 || m0 == 6 || m0 == 9 || m0 == 11) && d0 > 30)
        ) {
          alert("Invalid date444444444");
        }
        if (
          jgGOrJ == 2 ||
          (jgGOrJ == 0 &&
          ...
          if (y0 / 4.0 == Math.round(y0 / 4.0)) {
            if (y0 / 100.0 == Math.round(y0 / 100.0)) {
              if (y0 / 400.0 == Math.round(y0 / 400.0)) {
                /* Leap year */
                if (m0 == 2 && d0 > 29) {
                  alert("Invalid date6666666666");
                }
              }
            } else {
              /* Leap year */
              if (m0 == 2 && d0 > 29) {
                alert("Invalid date777777777");
              }
            }
          }
        } else {
          alert("Invalid date77777");
        }
        var jd =
      ...
      getLastDayOfPersianMonth: function (pd) {
        ...
        if (m >= 1 && m <= 6) {
          ...
          alert("Invalid date88888888888888");
        }
        /* Esfand */
        if (
  ...
  (function () {
    ...
    Date.prototype._toString = function (formatDate) {
      ...
      if (formatDate === undefined || formatDate == "default") return this;
      return formatDate
