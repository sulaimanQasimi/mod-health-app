...
import { ReportAnalytics, ReportChartCard, ReportKpiStat } from './reportTypes';
...
export interface PatientsReportTabData {
    patients: {
...
export default function PatientsReportPanel({
    data,
    urls,
    processing,
    onVisit,
    ...
    const handleSubmit = (event: FormEvent) => {
        ...
        const params: Record<string, string> = { tab: 'patients', search: '1' };
    ...
    const canExport = data.hasSearch && data.patients.data.length > 0;
    const kpiStats: ReportKpiStat[] = data.hasSearch
        ? [
              { key: 'total', label: t('global.total'), value: data.summary.total, icon: 'bx-user', accent: 'from-emerald-500 to-teal-600' },
              { key: 'male', label: t('global.male'), value: data.summary.male, icon: 'bx-male', accent: 'from-blue-500 to-cyan-600' },
              { key: 'female', label: t('global.female'), value: data.summary.female, icon: 'bx-female', accent: 'from-pink-500 to-rose-600' },
              { key: 'military', label: t('global.military'), value: data.summary.military, icon: 'bx-shield-quarter', accent: 'from-violet-500 to-purple-600' },
              { key: 'civilian', label: t('global.civilian'), value: data.summary.civilian, icon: 'bx-briefcase', accent: 'from-amber-500 to-orange-600' },
          ]
        : [];
    const charts: ReportChartCard[] = data.hasSearch
        ? [
              {
                  key: 'gender',
                  ...
                  labels: (data.analytics.by_gender ?? []).map((item) =>
                      item.name === 'female' ? t('global.female') : item.name === 'male' ? t('global.male') : '—',
                  ),
                  values: (data.analytics.by_gender ?? []).map((item) => item.count),
                  colors: ['#06b6d4', '#ec4899', '#94a3b8'],
              },
              {
                  key: 'type',
                  title: t('global.disease_type'),
                  type: 'bar',
                  labels: (data.analytics.by_type ?? []).map((item) =>
                      item.name === 'mod'
                          ? t('global.mod')
                          : item.name === 'recipient'
                            ? t('global.recipient')
                  ...
                  values: (data.analytics.by_type ?? []).map((item) => item.count),
                  color: '#10b981',
              },
              {
                  key: 'date',
                  title: t('global.registration_date'),
                  type: 'trend',
                  labels: (data.analytics.by_date ?? []).map((item) => item.date),
                  values: (data.analytics.by_date ?? []).map((item) => item.count),
                  color: '#6366f1',
              },
          ]
        : [];
    ...
    return (
        <div className="space-y-5">
            ...
            {data.hasSearch ? <ReportKpiGrid stats={kpiStats} /> : null}
            {data.hasSearch ? <ReportAnalyticsSection title={t('global.reports')} charts={charts} /> : null}
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    ...
                    <div>
                        ...
                        <SearchableSelect
                            ...
                            options={[
                                ...
                                { value: '3', label: t('global.extraordinary') },
                                { value: '4', label: t('global.vip') },
                            ]}
                        />
                    </div>
                    <div>
                        ...
                        <SearchableSelect
                            ...
                            options={[
                                { value: '', label: t('global.all') },
                                ...data.filterOptions.recipients.map((recipient) => ({
                    ...
                    <div>
                        ...
                        <SearchableSelect
                            ...
                            options={[
                                { value: '', label: t('global.all') },
                                ...data.filterOptions.provinces.map((province) => ({
            ...
            <ReportResultsCard
                ...
                resultCount={data.patients.meta.total}
            ...
            >
                    ...
                    <Table embedded>
                        ...
                        <TableBody>
                            {data.patients.data.length === 0 ? (
                                ...
                                data.patients.data.map((item, index) => (
                                    <TableRow key={item.id}>
                                        <TableCell className="font-mono text-xs text-gray-500">
                                            {(data.patients.meta.from ?? 1) + index}
                                        </TableCell>
                ...
                {data.patients.links.length > 3 && (
                    <AppointmentPagination links={data.patients.links} meta={data.patients.meta} t={t} />
                )}
            </ReportResultsCard>
        </div>
    );
