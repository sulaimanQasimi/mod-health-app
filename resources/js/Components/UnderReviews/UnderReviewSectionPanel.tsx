...
interface UnderReviewSectionPanelProps {
    id: string;
    icon: string;
    title: string;
    count?: number;
...
export function UnderReviewDataTable({ children }: { children: ReactNode }) {
    return (
...
export function UnderReviewDataTableHead({ children }: { children: ReactNode }) {
    return (
        <thead>
...
export function UnderReviewDataTableTh({
    children,
    className = '',
}: {
    children: ReactNode;
...
export function UnderReviewDataTableBody({ children }: { children: ReactNode }) {
    return <tbody className="divide-y divide-gray-100 dark:divide-gray-800">{children}</tbody>;
}

export function UnderReviewDataTableRow({ children }: { children: ReactNode }) {
    return (
        <tr className="bg-white transition-colors hover:bg-gray-50/50 dark:bg-gray-900 dark:hover:bg-gray-800/40">
            {children}
        </tr>
    );
}

export function UnderReviewDataTableTd({
    children,
    className = '',
    colSpan,
}: {
...
export default function UnderReviewSectionPanel({
    id,
    icon,
    title,
    count,
        ...
        <section className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-900">
            ...
            >
                <span className="flex min-w-0 items-center gap-3">
                    ...
                    {count !== undefined && (
                        <span className="shrink-0 rounded-full bg-gray-100 px-2 py-0.5 text-[11px] font-medium text-gray-600 ring-1 ring-gray-200 dark:bg-gray-800 dark:text-gray-300 dark:ring-gray-700">
                            {count}
                        </span>
                    )}
                </span>
