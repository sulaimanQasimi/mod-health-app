...
export default function IndexFoodTypes({
    foodTypes,
    filters: serverFilters,
    permissions,
    urls,
}: {
    foodTypes: PaginatedResult<FoodTypeItem>;
    ...
    const summaryLabel =
        foodTypes.meta.from && foodTypes.meta.to
            ? `${t('global.showing')} ${foodTypes.meta.from}-${foodTypes.meta.to} ${t('global.of')} ${foodTypes.meta.total}`
            : `${foodTypes.meta.total} ${t('global.results')}`;
                ...
                <Card className="shadow-sm">
                    ...
                    {foodTypes.data.length > 0 ? (
                        <Table>
                            ...
                            <TableBody>
                                {foodTypes.data.map((item, index) => (
                                    <TableRow key={item.id}>
                                        <TableCell>{(foodTypes.meta.from ?? 1) + index}</TableCell>
                    ...
                    {foodTypes.links.length > 0 && (
                        <ul className="mt-6 inline-flex -space-x-px text-sm">
                            {foodTypes.links.map((link, index) => renderPaginationLink(link, index))}
                        </ul>
                    )}
                </Card>
            </div>
