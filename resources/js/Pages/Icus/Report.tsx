...
export default function IcusReport({ items, hasSearch, summary, analytics, filters, urls }: ReportProps) {
    ...
    return (
        <ReportPageShell title={t('global.reports')} subtitle={t('global.new_icus')} accent="from-violet-600 to-purple-700" backLabel={t('global.back')}
            ...
            {hasSearch ? <ReportKpiGrid stats={[
                ...
                { key: 'new', label: t('global.new_icus'), value: summary.new, icon: 'bx-time-five', accent: 'from-amber-500 to-orange-600' },
                { key: 'approved', label: t('global.approved_icus'), value: summary.approved, icon: 'bx-check-circle', accent: 'from-emerald-500 to-teal-600' },
                { key: 'rejected', label: t('global.rejected_icus'), value: summary.rejected, icon: 'bx-x-circle', accent: 'from-rose-500 to-red-600' },
            ]} /> : null}
            {hasSearch ? <ReportAnalyticsSection title={t('global.reports')} charts={[
                { key: 'status', title: t('global.status'), type: 'donut', labels: analytics.by_status.map((item) => item.name === 'new' ? t('global.new_icus') : item.name === 'approved' ? t('global.approved_icus') : t('global.rejected_icus')), values: analytics.by_status.map((item) => item.count), colors: ['#0ea5e9', '#10b981', '#ef4444'] },
