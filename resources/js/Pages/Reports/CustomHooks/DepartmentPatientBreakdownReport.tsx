...
export interface DepartmentBreakdownReportRow {
...
interface DepartmentPatientBreakdownReportProps {
    endpoint: string;
...
function buildAllColumnsFromReport(
    report: DepartmentBreakdownReportRow[],
    t: (key: string) => string,
...
const DepartmentPatientBreakdownReport: React.FC<DepartmentPatientBreakdownReportProps> = ({
    endpoint,
    exportFileName,
    branch_id = '',
    date_from = '',
    ...
    const [report, setReport] = useState<DepartmentBreakdownReportRow[]>([]);
    ...
    useEffect(() => {
        ...
        get(`${endpoint}?${params.toString()}`)
            .then((response) => {
                const payload = response as { data?: DepartmentBreakdownReportRow[] };
    ...
    }, [branch_id, date_from, date_to, endpoint]);
    ...
    const getCount = (
        department: DepartmentBreakdownReportRow,
    ...
    const getColumnCount = (department: DepartmentBreakdownReportRow, columnId: string): number => {
    ...
    const exportData = useMemo(
        ...
        [
            ...
            grandTotal,
            t,
            tableSettings.showTotalsRow,
            visibleColumnGroups,
        ],
    );

    const removeDepartment = (department: DepartmentBreakdownReportRow) => {
        setReport((current) =>
            current.filter((row) => {
...
export default DepartmentPatientBreakdownReport;
