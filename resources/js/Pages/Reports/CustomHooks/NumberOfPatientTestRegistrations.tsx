...
interface TestRegistrationReport extends TestRegistrationApiRow {
...
interface NumberOfPatientTestRegistrationsProps {
...
function normalizeTestReportRow(row: TestRegistrationApiRow): TestRegistrationReport {
    return {
        ...row,
        department_id: row.lab_type_id,
        department_name: row.test_name,
    };
}

function buildAllColumnsFromReport(
    report: TestRegistrationReport[],
    t: (key: string) => string,
    ...
    return BREAKDOWN_CONFIGS.flatMap((config) => {
        ...
        report.forEach((testRow) => {
            testRow[config.key].forEach((item) => {
...
const NumberOfPatientTestRegistrations: React.FC<NumberOfPatientTestRegistrationsProps> = ({
    branch_id = '',
    date_from = '',
    date_to = '',
}) => {
    ...
    const testNameOptions = useMemo(() => buildDepartmentOptions(report), [report]);
    ...
    const getCount = (testRow: TestRegistrationReport, breakdownKey: BreakdownKey, itemKey: string): number => {
        const match = testRow[breakdownKey].find((item) => getItemKey(item) === itemKey);
        return match?.count ?? 0;
    };

    const getColumnCount = (testRow: TestRegistrationReport, columnId: string): number => {
        const [groupKey, itemKey] = columnId.split(':') as [BreakdownKey, string];
        return getCount(testRow, groupKey, itemKey);
    };
    ...
    const { columnTotals, grandTotal } = useMemo(() => {
        ...
        displayReport.forEach((testRow) => {
            total += testRow.count;

            flatColumns.forEach((column) => {
                const count = getCount(testRow, column.breakdownKey, column.itemKey);
    ...
    const exportData = useMemo(
        () =>
            buildGeneralReportExportData({
                fileName: 'patient-test-registrations',
                indexLabel: '#',
                rowLabelColumn: t('global.test_name'),
                totalLabel: t('global.total'),
                visibleColumnGroups,
                displayReport,
                getRowLabel: (testRow) => testRow.test_name ?? 'Unknown',
                getCellValue: (testRow, columnId) => getColumnCount(testRow, columnId),
                getRowTotal: (testRow) => testRow.count,
    ...
    const removeTestRow = (testRow: TestRegistrationReport) => {
        setReport((current) =>
            current.filter((row) => {
                if (testRow.lab_type_id != null && row.lab_type_id != null) {
                    return row.lab_type_id !== testRow.lab_type_id;
                }

                return row.test_name !== testRow.test_name;
            }),
        );
    };
    ...
    return (
        <>
                ...
                <table className="general-report-data-table w-full table-auto border-collapse divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody>
                        {displayReport.length > 0 ? (
                            displayReport.map((testRow, index) => (
                                <tr
                                    key={testRow.lab_type_id ?? testRow.test_name ?? index}
                                    className="border-t border-gray-200 dark:border-gray-700"
                                >
                                    <td className="sticky left-0 z-10 bg-white px-2 py-2 dark:bg-gray-800">
                                        <button
                                            type="button"
                                            onClick={() => removeTestRow(testRow)}
                                            title={t('global.remove')}
                                            aria-label={`${t('global.remove')} ${testRow.test_name ?? 'Unknown'}`}
                                    ...
                                    {visibleColumnGroups.map((group) =>
                                        group.columns.map((column, columnIndex) => (
                                            ...
                                            >
                                                {getCount(testRow, group.key as BreakdownKey, column.itemKey)}
                                            </td>
                                        )),
                                    )}
            ...
            <GeneralReportTableSettingsModal
                ...
                departmentOptions={testNameOptions}
                minTotalOptions={minTotalOptions}
                rowFilterLabel={t('global.test_name')}
                primarySortLabel={t('global.test_name')}
...
export default NumberOfPatientTestRegistrations;
