        ...
        <div class="container-fluid flex-grow-1 container-p-y">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">{{ localize('global.depot.depot_to_pharmacy_movement') }}</h5>
                    <a href="{{ route('depots.transactions.index') }}"
                        class="btn btn-outline-secondary btn-sm">{{ localize('global.depot.back') }}</a>
                </div>
                <div class="card-body">
                    <form action="{{ route('depots.movements.depot_to_pharmacy.store') }}" method="POST" autocomplete="off"
                        onsubmit="return confirm('{{ localize('global.move_stock_from_depot_to_pharmacy') }}')">
                        @csrf
                        <div class="row g-3">
                            <div class="col-md-6">
                                <label class="form-label"
                                    for="from_depot_id">{{ localize('global.depot.source_depot') }}</label>
                                <select name="from_depot_id" id="from_depot_id"
                                    class="form-select select2 @error('from_depot_id') is-invalid @enderror" required>
                                    <option value="">{{ localize('global.depot.select_source_depot') }}</option>
                                    @foreach ($depots as $depot)
                                        <option value="{{ $depot->id }}" @selected(old('from_depot_id') == $depot->id)>{{ $depot->name }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('from_depot_id')
                            ...
                            <div class="col-md-6">
                                <label class="form-label" for="pharmacy_id">{{ localize('global.pharmacy') }}</label>
                                <select name="pharmacy_id" id="pharmacy_id"
                                    class="form-select select2 @error('pharmacy_id') is-invalid @enderror" required>
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="medicine_id" id="medicine_id"
                                    class="form-select select2 @error('medicine_id') is-invalid @enderror" required>
                            ...
                            <div class="col-md-4">
                                <label class="form-label"
                                    for="transaction_date">{{ localize('global.transaction_date') }}</label>
                        ...
                        <div class="d-flex justify-content-end gap-2 mt-4">
                            <button type="reset"
                                class="btn btn-outline-secondary">{{ localize('global.reset') }}</button>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function refreshStock() {
                ...
                fetch(
                        `{{ route('depots.stock.available') }}?depot_id=${depot.value}&medicine_id=${medicine.value}`)
