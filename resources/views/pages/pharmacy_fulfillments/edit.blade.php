            ...
            <div class="col-xl">
                <div class="card mb-4">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h5 class="mb-0">{{ localize('global.edit_pharmacy_fulfillment') }}</h5>
                    </div>
                    <div class="card-body">
                        @if($userPharmacy)
                        ...
                        @endif
                        <form method="POST" action="{{ route('pharmacy_fulfillments.update', $pharmacyFulfillment->id) }}" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="row">
                                <div class="col-md-6">
                                        ...
                                        <select class="form-control select2 @error('medicine_id') is-invalid @enderror"
                                            ...
                                            @foreach ($medicines as $medicine)
                                                <option value="{{ $medicine->id }}"
                                                    {{ old('medicine_id', $pharmacyFulfillment->medicine_id) == $medicine->id ? 'selected' : '' }}>
                                                    {{ $medicine->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group mb-3">
                                        ...
                                        <input type="text" class="form-control @error('unit_type') is-invalid @enderror"
                                            id="unit_type" name="unit_type" value="{{ old('unit_type', $pharmacyFulfillment->unit_type) }}" placeholder="{{ localize('global.unit_type') }}">
                                        @error('unit_type')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group mb-3">
                                        ...
                                        <input type="text" class="form-control @error('amount') is-invalid @enderror"
                                            id="amount" name="amount" value="{{ old('amount', $pharmacyFulfillment->amount) }}" required>
                                        @error('amount')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group mb-3">
                                        ...
                                        <input type="text" class="form-control @error('form_no') is-invalid @enderror"
                                            id="form_no" name="form_no" value="{{ old('form_no', $pharmacyFulfillment->form_no) }}" required>
                                        @error('form_no')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group mb-3">
                                        ...
                                        <input type="text" autocomplete="off" class="form-control datepicker_dari @error('date') is-invalid @enderror"
                                            id="date" name="date" value="{{ old('date', $pharmacyFulfillment->date ? \Verta::instance($pharmacyFulfillment->date)->format('Y-m-d') : '') }}" required placeholder="{{ localize('global.date') }}">
                                        @error('date')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
                                        ...
                                        @if($pharmacyFulfillment->form)
                                            <div class="mb-2">
                                                <a href="{{ Storage::disk('public')->url($pharmacyFulfillment->form) }}" target="_blank" class="btn btn-sm btn-info">
                                                    <i class="bx bx-file"></i> {{ localize('global.view_current_pdf') }}
                                                </a>
                                            </div>
                                        @endif
                                        <input type="file" class="form-control @error('form') is-invalid @enderror"
                                            id="form" name="form" accept=".pdf,application/pdf">
                                        <small class="form-text text-muted">{{ localize('global.max_file_size_10mb') }}. {{ localize('global.leave_empty_to_keep_current') }}</small>
                                        @error('form')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
