            ...
            <div class="col-xl">
                <div class="card mb-4">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h5 class="mb-0">{{ localize('global.edit_room') }}</h5>
                    </div>
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" name="name" id="name" value="{{ old('name', $room->name) }}" class="form-control">
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="mb-3">
                                        <label for="floor_id">{{ localize('global.floor') }}</label>
                                        <select class="form-control select2" name="floor_id">
                                            <option value="">{{ localize('global.select') }}</option>
                                            @foreach($floors as $value)
                                                <option value="{{ $value->id }}"
                                                    {{ old('floor_id', $room->floor_id) == $value->id ? 'selected' : '' }}>
                                                {{ $value->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-control select2" name="department_id">
                                            ...
                                            @foreach($departments as $value)
                                                <option value="{{ $value->id }}"
                                                    {{ old('department_id', $room->department_id) == $value->id ? 'selected' : '' }}>
                                                {{ $value->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
