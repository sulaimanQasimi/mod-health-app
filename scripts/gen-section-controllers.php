...
class BloodBankController extends Controller
{
    ...
    public function store(Request $request, Appointment $appointment): JsonResponse
    {
        ...
        return response()->json(['success' => true, 'message' => localize('global.blood_request_created_successfully'), 'data' => ['id' => $item->id]]);
    }

    public function destroy(Appointment $appointment, BloodBank $bloodBank): JsonResponse
    {
        $this->authorizeAppointmentView($appointment);
        abort_unless($bloodBank->appointment_id === $appointment->id, 404);
        $bloodBank->delete();

        return response()->json(['success' => true, 'message' => localize('global.blood_request_deleted_successfully')]);
    }
}

PHP;

file_put_contents($dir.'/BloodBankController.php', $controllers);

echo "Generated BloodBankController\n";
